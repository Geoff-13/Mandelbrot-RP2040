...
//
// The RGB_m() macro can be used to create colors from this colour depth using 8-bits per
// channel values.
...
const uint16_t palette[8] = {
    ...
    RGB_m(205, 0, 0),    // Red
    RGB_m(0, 205, 0),    // Green
    RGB_m(205, 205, 0),  // Yellow
    RGB_m(0, 0, 238),    // Blue
    RGB_m(205, 0, 205),  // Magenta
    RGB_m(0, 205, 205),  // Cyan
    RGB_m(229, 229, 229) // White
};

// Bright colours for the 3-bit terminal colours (based on VS Code)
const uint16_t bright_palette[8] = {
    RGB_m(127, 127, 127), // Bright Black (Gray)
    RGB_m(255, 0, 0),     // Bright Red
    RGB_m(0, 255, 0),     // Bright Green
    RGB_m(255, 255, 0),   // Bright Yellow
    RGB_m(92, 92, 255),   // Bright Blue
    RGB_m(255, 0, 255),   // Bright Magenta
    RGB_m(0, 255, 255),   // Bright Cyan
    RGB_m(255, 255, 255)  // Bright White
};
...
void (*term_bell_callback)(void) = NULL;
void (*term_report_callback)(const char *) = NULL;
...
static void ring_bell() {
    if (term_bell_callback) {
        term_bell_callback(); // Call the user-defined bell callback
    }
}

static void report(const char *msg) {
    if (term_report_callback) {
        term_report_callback(msg);
    }
}
...
void term_set_bell_callback(bell_callback_t callback) {
    term_bell_callback = callback;
}

void term_set_report_callback(report_callback_t callback) {
    term_report_callback = callback;
}
