...
static col565_t foreground = 0xFFFF; // default foreground colour (white)
static col565_t background = 0x0000; // default background colour (black)
...
static uint16_t line_buffer[LCD_WIDTH * GLYPH_HEIGHT] __attribute__((aligned(4)));
...
void lcd_write16_buf(const col565_t *buffer, size_t len) {
    // DO NOT MOVE THE spi_set_format() OR THE gpio_put(LCD_DCX) CALLS!
    // They are placed before the gpio_put(LCD_CSX) to ensure that a minimum
    // chip select high pulse width is achieved (at least 40ns)
    spi_set_format(LCD_SPI, 16, 0, 0, SPI_MSB_FIRST);
...
void lcd_blit_rect(const col565_t *pixels, int16_t x, int16_t y, int16_t width, int16_t height) {
    ...
    if (y >= lcd_scroll_top && y < LCD_HEIGHT - lcd_scroll_bottom) {
        // Adjust y for vertical scroll offset and wrap within memory height
        int16_t y_virtual = (lcd_y_offset + y) % lcd_memory_scroll_height;
        int16_t y_end = lcd_scroll_top + y_virtual + height - 1;
    ...
    lcd_write16_buf((col565_t *)pixels, width * height);
    lcd_enable_interrupts();
}

//    _____________
// __/ Write pixel \_______________________________________________________________________________
void  lcd_write_pixel(const col565_t colour, int16_t x, int16_t y) {

    lcd_disable_interrupts();
        lcd_set_window(x, y, x, y);
        lcd_write16_data(1, colour);
    lcd_enable_interrupts();
}
...
static void lcd_solid_rectangle(col565_t colour, int16_t x, int16_t y, int16_t width, int16_t height) {
    static col565_t pixels[LCD_WIDTH];

    for (uint16_t row = 0; row < height; row++) {
        for (uint16_t i = 0; i < width; i++) {
            pixels[i] = colour;
        }
        lcd_blit_rect(pixels, x, y + row, width, 1);
    }
}
...
//  set the vertical scrolling area of the display, but it is the responsibility of lcd_blit_rect()
...
void lcd_define_scrolling(int16_t top_fixed_area, int16_t bottom_fixed_area) {
    int16_t scroll_area = LCD_HEIGHT - (top_fixed_area + bottom_fixed_area);
    ...
    lcd_write_data(6,
                   MSB_m(lcd_scroll_top),
                   LSB_m(lcd_scroll_top),
                   MSB_m(scroll_area),
                   LSB_m(scroll_area),
                   MSB_m(lcd_scroll_bottom),
                   LSB_m(lcd_scroll_bottom));
    lcd_enable_interrupts();
...
void lcd_scroll_reset() {
    ...
    int16_t scroll_area_start = lcd_scroll_top + lcd_y_offset;

    lcd_disable_interrupts();
    lcd_write_cmd(LCD_CMD_VSCSAD); // Sets where in display RAM the scroll area starts
    lcd_write_data(2, MSB_m(scroll_area_start), LSB_m(scroll_area_start));
    lcd_enable_interrupts();
}
...
void lcd_scroll_clear() {
    ...
    lcd_solid_rectangle(background, 0, lcd_scroll_top, LCD_WIDTH, lcd_memory_scroll_height);
}
...
void lcd_scroll_up() {
    ...
    lcd_solid_rectangle(background, 0, LCD_HEIGHT - GLYPH_HEIGHT, LCD_WIDTH, GLYPH_HEIGHT);
}
...
void lcd_scroll_down() {
    ...
    lcd_solid_rectangle(background, 0, lcd_scroll_top, LCD_WIDTH, GLYPH_HEIGHT);
}
...
void lcd_clear_screen() {
    ...
    lcd_solid_rectangle(background, 0, 0, LCD_WIDTH, FRAME_HEIGHT);
}
...
void lcd_putc(uint8_t column, uint8_t row, uint8_t c) {
    ...
    lcd_blit_rect(char_buffer, column * font->width, row * GLYPH_HEIGHT, font->width, GLYPH_HEIGHT);
}
...
void lcd_putstr(uint8_t column, uint8_t row, const char *str) {
    ...
    if (len) {
        lcd_blit_rect(line_buffer, column * font->width, row * GLYPH_HEIGHT, font->width * len, GLYPH_HEIGHT);
    }
}
...
void lcd_init() {
    ...
    lcd_write_cmd(LCD_CMD_EM); // entry mode set
